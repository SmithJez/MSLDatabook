...
class FilterSkillAdapter (var context: Context,
                          skillsEffect: List<MsgStatusEffect>,
    ...
    val _saveType = saveType

    var listSkillEffect: List<MsgStatusEffect> = listOf()
    var ctx : Context
    ...
    init {
        ...
        arrayCheck = (context as FilterMonActivity).ReadSavedFilter(saveType)
        ...
        if (CountCheck()) {
            if (saveType == Variables.SKILL_LEADER) {
                (fragment as FilterGeneralFragment).CheckedSelectAll(Variables.SKILL_LEADER)
            } else {

                if(saveType == Variables.SKILL_DEFAULT) {
                    (fragment as FilterPassiveFragment).CheckedSelectAll()
                } else {
                    (fragment as FilterActiveFragment).CheckedSelectAll()
                }
            ...
            if (saveType == Variables.SKILL_LEADER) {
                ...
                if(saveType == Variables.SKILL_DEFAULT) {
                    (fragment as FilterPassiveFragment).UnCheckedSelectAll()
                } else {
                    (fragment as FilterActiveFragment).UnCheckedSelectAll()
                }
            }
        }
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        ...
        skill = listSkillEffect[position]
        mapEff = mapFx[skill.uid]!!
        skillName = mapStr[ mapEff.name  ]!!

        skillPath = GetSkillEffectResource( mapFx[skill.uid]!!.subType.name)
